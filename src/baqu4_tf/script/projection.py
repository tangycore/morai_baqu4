...
parameters_cam = {
    ...
    "Y": 0.0,
    "Z":  1.400,
    "YAW": 0.0 * math.pi / 180.0, # radian
    "PITCH": 0.0 * math.pi / 180.0, # rot.pitch=355° → -5°
    "ROLL":  0.0 * math.pi / 180.0
}
parameters_lidar = {
    "X": 0.0, # meter
    "Y": 0.0,
    "Z": 0.0,
    "YAW": 0.0, # - 7.5*math.pi/180.0, # radian
    "PITCH": 0.0,
    "ROLL": 0.0
}
def getRotMat(RPY):
...
def getTransformMat(params_cam, params_lidar):
    ...
    print(f"Original Camera RPY (rad): {camRPY}")
    print(f"Original Camera RPY (deg): {np.rad2deg(camRPY)}")

    camRPY = camRPY + np.array([-90*math.pi/180, 0, -90*math.pi/180])


    print(f"Modified Camera RPY (rad): {camRPY}")
    print(f"Modified Camera RPY (deg): {np.rad2deg(camRPY)}")
...
class LiDARToCameraTransform:
    def __init__(self, params_cam, params_lidar):        
        self.scan_sub = rospy.Subscriber("/front_lidar/velodyne_points", PointCloud2, self.scan_callback)
